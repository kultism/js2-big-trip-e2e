Feature: 1.6 Фильтры
  ...
  Scenario: Выбор любого из фильтров приводит к отрисовке только соответствующих точек
    ...
    Then элемент '[id="filter-present"i]:checked' есть на странице
    Then на странице '1' элементов '.event'
    When кликаю по элементу '[for="filter-future"i]'
    Then элемент '[id="filter-future"i]:checked' есть на странице
    Then на странице '1' элементов '.event'
    When кликаю по элементу '[for="filter-everything"i]'
    Then элемент '[id="filter-everything"i]:checked' есть на странице
    Then на странице '5' элементов '.event'
