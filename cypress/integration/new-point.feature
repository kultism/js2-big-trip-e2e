Feature: 1.4 Новая точка маршрута
  ...
  Background:
    ...
    Then на странице '5' элементов '.trip-events__item'
  ...
  Scenario: Новая точка маршрута создаётся нажатием на кнопку «New Event»
    ...
    Then на странице '6' элементов '.trip-events__item'
  ...
  Scenario: Если был выбран фильтр или применена сортировка, то они сбрасываются
    ...
    Then элемент '[value="past"i]:checked' есть на странице
    When кликаю по элементу '[for="sort-price"i]'
    Then элемент '[value="sort-price"i]:checked' есть на странице
    When кликаю по элементу '.trip-main__event-add-btn'
    Then элемента '[value="sort-price"i]:checked' нет на странице
    Then элемента '[value="sort-price"i]:checked' нет на странице
    Then элемент '[value="everything"i]:checked' есть на странице
