Feature: 1.9 Обратная связь интерфейса
  ...
  Scenario: На время выполнения запроса текст заголовка кнопки изменяется
    ...
    When кликаю по элементу '.trip-events__item:nth-child(5) .event__rollup-btn'
  ...
  Scenario: В момент отправки запроса на создание точки форма блокируется
    ...
    When заполняю форму
    When кликаю по элементу '.event__save-btn'
    Then элемент '.event__type-toggle' содержит атрибут 'disabled'
    Then элемент '.event__input--destination' содержит атрибут 'disabled'
    Then элемент '[id*="event-start-time"i]' содержит атрибут 'disabled'
    Then элемент '[id*="event-end-time"i]' содержит атрибут 'disabled'
    Then элемент '.event__input--price' содержит атрибут 'disabled'
    Then элемент '.event__save-btn' содержит атрибут 'disabled'
    Then элемент '.event__reset-btn' содержит атрибут 'disabled'
    When запрос на создание точки завершён
    Then элемент '.event__type-toggle' не содержит атрибут 'disabled'
    Then элемент '.event__input--destination' не содержит атрибут 'disabled'
    Then элемент '[id*="event-start-time"i]' не содержит атрибут 'disabled'
    Then элемент '[id*="event-end-time"i]' не содержит атрибут 'disabled'
    Then элемент '.event__input--price' не содержит атрибут 'disabled'
    Then элемент '.event__save-btn' не содержит атрибут 'disabled'
    Then элемент '.event__reset-btn' не содержит атрибут 'disabled'

  Scenario: В момент отправки запроса на удаление точки форма блокируется
  ...
  Scenario: Добавление в избранное происходит только после успешного выполнения запроса к серверу
    ...
    Given нахожусь на главной странице сайта
    When запрос на получение данных завершён
    When кликаю по элементу '.trip-events__item:nth-child(3) .event__favorite-btn'
    When запрос на изменение точки завершён
    Then точка '3' не содержит элемент '.event__favorite-btn--active'
    When кликаю по элементу '.trip-events__item:nth-child(4) .event__favorite-btn'
    When запрос на изменение точки завершён
  ...
  Scenario: Если не удалось выполнить запрос, точка «покачивает головой»
    ...
    When кликаю по элементу '.trip-events__item:nth-child(3) .event__favorite-btn'
    When запрос на изменение точки завершён
    Then элемент '.trip-events__item:nth-child(3)' "качает головой"
